...
public class AuthenticationCheckController {
    ...
    public User checkAuthentication(HttpServletResponse response) {
        ...
        if ((SecurityContextHolder.getContext().getAuthentication() != null && SecurityContextHolder.getContext().getAuthentication().isAuthenticated()
                && SecurityContextHolder.getContext().getAuthentication().getAuthorities().contains(ROLE_ANONYMOUS)) || (facebook == null)){
            ...
            User user = userRepository.findByName(((SocialUserDetails) SecurityContextHolder.getContext().getAuthentication().getPrincipal()).getUserId());
            if (user == null) {
                throw new InternalError("Cannot find user by username ");
            }
            return user;
        }
